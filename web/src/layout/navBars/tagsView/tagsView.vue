...
const solveAddTagsView = async (path: string, to?: RouteToFrom) => {
	...
	let toQueryParams = to?.meta?.isDynamic ? to?.params || {} : to?.query || {};

	// 查找相同路径的标签页（不比较参数）
	let existingTag = state.tagsViewList.find((v: RouteItem) => v.path === isDynamicPath);

	if (existingTag) {
		// 如果找到相同路径的标签页，更新其参数和URL
		to?.meta?.isDynamic ? (existingTag.params = to.params) : (existingTag.query = to?.query);
		existingTag.url = setTagsViewHighlight(existingTag);
