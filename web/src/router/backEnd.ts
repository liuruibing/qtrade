...
export async function initBackEndControlRoutes() {
	...
	let processedRoutes = await backEndComponent(frameIn);
	
	// 确保 /home 路由始终指向 dashboard，移除后端返回的 /home 路由（如果存在）
	processedRoutes = processedRoutes.filter((route: any) => route.path !== '/home' && route.name !== 'home');
	
	// 添加硬编码的 /home 路由指向 dashboard
	const homeRoute = {
		path: '/home',
		name: 'home',
		component: dynamicImport(dynamicViewsModules, '/AI/dashboard'),
		meta: {
	...
	dynamicRoutes[0].children = [homeRoute, ...processedRoutes];
	// 添加动态路由
	await setAddRoute();
