	...
	<div style="width: 100%" :class="props.class" :style="props.style">
		<slot name="input" v-bind="{}">
			<div v-if="props.showInput" style="width: 100%" :class="props.inputClass" :style="props.inputStyle">
				<el-select
					v-if="props.inputType === 'selector'"
					v-model="data"
					suffix-icon="arrow-down"
					clearable
					:multiple="props.multiple"
					placeholder="请选择文件"
					@click="selectVisiable = true && !props.disabled"
				...
				<div v-if="props.inputType === 'image' && props.multiple" style="width: 100%; display: flex; gap: 4px; flex-wrap: wrap; margin-bottom: 4px">
					<div
						v-for="(item, index) in data || []"
						style="position: relative"
						:style="{ width: props.inputSize + 'px', height: props.inputSize + 'px' }"
					>
						<el-image :src="item" :key="index" fit="scale-down" class="itemList" :style="{ width: props.inputSize + 'px', aspectRatio: '1 / 1' }" />
					...
					<div style="position: relative" :style="{ width: props.inputSize + 'px', height: props.inputSize + 'px' }">
						...
						<div
							@click="selectVisiable = true && !props.disabled"
							class="addControllorHover"
							:style="{ cursor: props.disabled ? 'not-allowed' : 'pointer' }"
						></div>
					</div>
				</div>
				<div
					v-if="props.inputType === 'image' && !props.multiple"
					class="form-display"
					style="position: relative"
					@mouseenter="formDisplayEnter"
				...
				>
					<video
						...
						:muted="true"
						:loop="true"
						:style="{ maxWidth: props.inputSize * 2 + 'px', maxHeight: props.inputSize + 'px', margin: '0 auto' }"
					></video>
					<div
						v-show="!!!data"
						style="position: absolute; left: 0; top: 0; width: 100%; height: 100%; display: flex; justify-content: center; align-items: center"
				...
				<div
					...
					:style="{ width: props.inputSize * 2 + 'px', height: props.inputSize + 'px' }"
				>
					<audio :src="data" :controls="!!data" :autoplay="false" :muted="true" :loop="true" style="width: 100%; z-index: 1"></audio>
					<div
						v-show="!!!data"
						style="position: absolute; left: 0; top: 0; width: 100%; height: 100%; display: flex; justify-content: center; align-items: center"
					>
		...
		>
			...
			<div style="padding: 4px">
				<div style="width: 100%; display: flex; justify-content: space-between; gap: 12px">
					...
					<el-tabs style="width: 100%" v-model="tabsActived" :type="props.tabsType" :stretch="true" @tab-change="handleTabChange" v-if="isTenentMode">
					...
					<el-col :span="12">
						<slot name="actionbar-left">
							<el-input
								...
								:placeholder="`请输入${TypeLabel[tabsActived % 4]}名`"
								prefix-icon="search"
								clearable
								@change="listRequest"
							/>
							<div>
								<el-tag v-if="props.multiple" type="primary" effect="light"> 一共选中&nbsp;{{ data?.length || 0 }}&nbsp;个文件 </el-tag>
							</div>
						</slot>
					</el-col>
						...
						<slot name="actionbar-right" v-bind="{}">
							...
							<template v-if="tabsActived > 3 ? isSuperTenent : true">
								<el-upload
									...
									:show-file-list="true"
									:accept="AcceptList[tabsActived % 4]"
									:on-success="
										() => {
											listRequest();
											listRequestAll();
					...
					>
						<slot name="item" :data="item">
							<FileItem
								:fileData="item"
								:api="fileApi"
								:showClose="tabsActived < 4 || isSuperTenent"
								@onDelFile="
									listRequest();
									listRequestAll();
								"
			...
			<el-dialog
				...
				:align-center="false"
				:append-to-body="true"
				:title="'网络' + TypeLabel[tabsActived % 4] + '上传'"
				@closed="netUrl = ''"
				:close-on-click-modal="!netLoading"
				:close-on-press-escape="!netLoading"
				:show-close="!netLoading"
				modal-class="_overlay"
			>
...
const handlePageChange = (currentPage: number, pageSize: number) => {
	pageForm.page = currentPage;
	pageForm.limit = pageSize;
	listRequest();
};
// 选择的行为
...
const confirmNetUrl = () => {
	...
	fetch(netPrefix.value + netUrl.value, { signal: controller.signal })
