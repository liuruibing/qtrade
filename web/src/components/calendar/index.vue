				...
				<tbody>
					<tr v-for="week in calendarList">
						<td class="calender-td" v-for="item in week">
							<div
								class="calender-cell"
								:data-date="item.date.toLocaleDateString('en-CA')"
								:class="{
								...
								@mouseenter="onCalenderCellHover"
								@mouseleave="onCalenderCellUnhover"
							...
							>
								...
								<div class="calender-cell-body calender-cell-line">
									<slot name="cell-body" v-bind="item"> </slot>
								</div>
...
const calendarList = computed(() => {
	let dates =
		!!props.range && props.range[0] && props.range[1]
	...
	let proce_dates: CalendarCell[] = dates.map((value) => {
		...
		// 农历节日、国际节日、节气三选一
		let holiday = showHoliday.value
			? (showLunarHoliday.value ? lunarHolidays[0] : '') ||
			  (showJieQi.value ? jieQi : '') ||
			  (showDetailedHoliday.value ? solarHolidays[0] : yearHolidays.value[value.toLocaleDateString('en-CA')])
			: ''; // yearHolidays国际的
		return {
			date: value,
...
const turnToNextY = () =>
	(currentCalendarDate.value = new Date(currentCalendarDate.value.getFullYear() + 1, currentCalendarDate.value.getMonth(), 1));
const turnToToday = () => (currentCalendarDate.value = new Date());
// 如果禁止跨页则跨页时清空选择
watch(
	() => currentCalendarDate.value,
	(v, ov) => (props.crossPage ? {} : v.toLocaleDateString('en-CA') === ov.toLocaleDateString('en-CA') ? {} : clear())
);
// 单元格事件
