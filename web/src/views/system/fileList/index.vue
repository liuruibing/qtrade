		...
		<fs-crud ref="crudRef" v-bind="crudBinding">
			<template #actionbar-left="scope">
				<el-upload
					...
					:multiple="false"
					:on-success="() => crudExpose.doRefresh()"
					:drag="false"
			...
			<template #cell_preview="scope">
				<div v-if="scope.row.file_type === 0">
					<el-image
						...
						:preview-src-list="[getBaseURL(scope.row.url)]"
						:preview-teleported="true"
					/>
				</div>
				<div v-if="scope.row.file_type === 1" class="_preview" @click="openPreviewHandle(getBaseURL(scope.row.url), 'video')">
					<el-icon :size="60">
						<VideoCamera />
					</el-icon>
				</div>
				<div v-if="scope.row.file_type === 2" class="_preview" @click="openPreviewHandle(getBaseURL(scope.row.url), 'video')">
					<el-icon :size="60">
						<Headset />
					</el-icon>
				</div>
